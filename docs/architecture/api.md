...
| POST | `/api/ai/generate-tests` | `{ projectId, kind: smoke\|crit-regression\|full, slugs[≤30], model?, negatives? }` → `{ cases[], dropped, missing[] }` (анти-галлюцинационная проверка по slug) | 400/5xx |
