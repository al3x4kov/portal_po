...
> ⚠️ Если вы скачали проект ZIP-архивом (кнопка «Code → Download ZIP»), **сначала
> полностью распакуйте архив**: правой кнопкой → «Извлечь всё…» — и запускайте
> `start.cmd` уже из распакованной папки. Двойной клик по `start.cmd` прямо в окне
> открытого архива не сработает: проводник распаковывает во временную папку только
> сам скрипт, без файлов проекта (типичный симптом — ошибка `Missing script: "build"`).
...
- Связи: `PARENT_OF`/`CHILD_OF` (иерархия), `RELATES_TO`, `DEPENDS_ON`, `BLOCKED_BY`.
  ...
  Диалог «Новая связь» из карточки открывается **поверх неё**: несохранённые правки
  карточки (название, описание, приоритизация и т. д.) при добавлении связи не теряются,
  а список связей в карточке обновляется сразу после создания.
...
Экспорт — **полноэкранный режим** (`/p/:id/export`), а не диалог: слева дерево с
чекбоксами, справа формат и живой состав выгрузки. Чекбокс раздела выбирает всю
ветку целиком (частичный выбор подсвечен), а панель справа на лету пересчитывает,
сколько ФТ, НФТ и связей попадёт в файл.

![Экспорт проекта: дерево с выбором ветками и живой состав выгрузки](docs/screenshots/export-screen.png)

- Архив `.zip` / `.tar.gz` (полный проект или только выбранные требования) —
  пригоден для повторного импорта; архивы с папкой-обёрткой поддерживаются.
- **Excel** (`.xlsx`) — лист «Требования», зеркалит таблицу портала; экран честно
  предупреждает, что обратно такой файл не импортируется.
- Набор выгружаемых полей настраивается тут же: три обязательных (название,
  критичность, статус) залочены, четыре опциональных переключаются, превью
  показывает итоговые колонки Excel и секции OpenSpec.
- Ошибка сервера не теряет работу: причина видна на экране, выбор и настройки
  сохранены, повтор — в один клик.
...
Генерация — тоже полноэкранный мастер (`/p/:id/generate`) с вертикальным
степпером: **направление → способ и параметры → результат**. На первом шаге у
каждой модели виден **живой бейдж охвата** («26 из 34 ФТ») и панель «Что вы
получите» с правилами отбора и превью структуры файла — масштаб понятен до
запуска. Если под правила отбора не попало ни одного ФТ, «Далее» блокируется,
а панель объясняет причину и ведёт на экран требований.

У smoke-модели между направлением и способом есть шаг **«Состав модели»**:
принцип отбора описан словами, каждое отобранное ФТ показано с причиной
попадания (высокая критичность / корень дерева / не реализовано), и любое
можно **исключить чекбоксом** — на следующий шаг, в смету и в итоговый файл
уйдёт только оставшееся. Сводка «отобрано правилом / исключено вручную /
войдут в модель» пересчитывается на лету, исключения сохраняются при
возврате «Назад», исключить все ФТ нельзя.

|                    Шаг 1: направление и охват                     |                 Шаг 2: способ и параметры                 |
| :---------------------------------------------------------------: | :-------------------------------------------------------: |
| ![Выбор направления генерации](docs/screenshots/testgen-mode.png) | ![Способ и параметры AI](docs/screenshots/testgen-ai.png) |

На втором шаге — **развилка способа** и все параметры сразу, без лишних переходов:
...
2. **AI-генерация по описаниям** — модель-QA пишет конкретные шаги по описаниям
   ...
   чекбокс «Добавлять негативные сценарии» и мини-смета «требований / батчей / времени».
   Клиент режет отбор на батчи по 6 требований; на каждый батч сервер делает
   вызов хаба (`POST /api/ai/generate-tests`, structured output с фолбэком).
   **Надёжность прогона** устроена как в AI-импорте документации: бюджет ответа
   считается от размера батча (кейс с негативным сценарием вдвое дороже) и
   клампится пресетом модели; батч, который заведомо не помещается в бюджет,
   режется ещё до вызова; 429/5xx/сеть/таймаут повторяются с backoff под
   per-call таймаутом; часть, которая всё равно не далась (обрыв ответа,
   нераспознанный JSON), делится пополам вплоть до одного требования. Сбойный
   батч не останавливает очередь — остальные проходят, а повторить можно
   точечно только неудавшиеся.
   Если AI не настроен, карточка AI заблокирована с причиной и прямым переходом
   в «Настройку AI», а путь через шаблон остаётся открытым.

**Фильтр по реализации.** У всех трёх тестовых моделей на шаге параметров есть
чекбокс «Включить нереализованные ФТ (N)» — он работает в обоих способах
(шаблон и AI, вместо отдельного шага-вопроса); снимите галочку, чтобы собрать
модель **только по реализованным ФТ** — охват и смета пересчитаются на лету.
В направлении «Задачи в TaskTracker» тот же явный отбор дают чипы фильтра
реализации в дереве выбора («Все / Реализовано / Запланировано», для ФТ и НФТ),
как и на экране экспорта.

Во время прогона кейсы появляются по мере ответов батчей, рядом — прогресс,
живой журнал и «Остановить». **Работа не теряется**: и остановка пользователем,
и ошибка батча сохраняют полученные кейсы и дают три выхода — продолжить с того
же батча, достроить остаток шаблоном или скачать частичный `.md`. Попытка уйти
со страницы во время прогона спрашивает подтверждение.
...
На экране результата кейсы показаны карточками с бейджем происхождения
(AI / шаблон) — видно, что именно достроено шаблоном, — а счётчики проверки и
журнал прогона стоят рядом с результатом; переключатель «карточки | markdown»
открывает сырой файл:

![Результат генерации: карточки кейсов, счётчики проверки и журнал прогона](docs/screenshots/testgen-preview.png)
...
- **AI-импорт документации:** архив с документацией продукта (до 200 МБ / 2000 файлов)
  ...
  смета токенов с подтверждением → извлечение → построение дерева → **двухзонная ручная
  выверка дублей** → запись; чекпоинты после каждого фрагмента, resume после сбоя без
  повторной оплаты, батчинг мелких файлов, адаптивные фрагменты и structured output для
  слабых моделей, русская таксономия ошибок с планом действий, итоговый отчёт качества
  и лог файлом.
- **Двухзонная выверка дублей (гейт PO):** до записи в проект не доходит ничего, что вы
  не подтвердили руками. **Зона 1** — дубли сгенерированных требований **между собой**:
  спорные пары больше не сливаются молча — модель лишь помечает их группами, а таблица
  выверки показывает все сгенерированные требования с предлагаемым местом в дереве и
  связями (полная картина «что оставить, что не включать»); в каждой группе по умолчанию
  оставлена одна запись. **Зона 2** (после апрува зоны 1) — дубли выбранного набора с
  **уже существующими** в проекте требованиями (точное имя и fuzzy-похожесть): найденные
  дубли по умолчанию исключены из записи, решение — за вами. Пауза выверки — REST-гейт:
  переживает перезапуск сервера, переоткрывается из истории прогонов, отмена на гейте
  ничего не пишет.
...
- **Чат по проекту** (виджет) и **генерация/дополнение описания** в карточке требования.
  В чате есть переключатель **«Учитывать требования проекта»**: ответы опираются на
  зафиксированные ФТ/НФТ. Контекст собирается детерминированно под бюджет токенов:
  маленький проект (10–15 требований) уходит в модель целиком, большой (1000–2000) —
  выборкой, релевантной вопросу, плюс обзор дерева именами; обрезка всегда помечается
  явно, дополнительных AI-вызовов и эмбеддингов не требуется.
...
flowchart TD
    ...
    G --> H["Дедупликация извлечённого<br/>полные повторы имён сливаются кодом ·<br/>спорные пары модель помечает ГРУППАМИ (не сливает)"]
    ...
    L --> M
    M --> R1[["Гейт PO · Зона 1: дубли сгенерированных<br/>МЕЖДУ СОБОЙ — группы + место в дереве + связи ·<br/>в группе по умолчанию оставлена одна запись"]]
    R1 -- "окно закрыто" --> R1P(["Пауза сохранена — выверка<br/>переоткрывается из истории (и после рестарта)"])
    R1 -- "отмена" --> X2(["Отменено — в проект<br/>ничего не записано"])
    R1 -- "подтверждено" --> S["Анализ дублей с УЖЕ существующими в проекте<br/>точное имя + fuzzy-похожесть · детерминированно, без AI"]
    S --> R2[["Гейт PO · Зона 2: дубли с проектом (duplicateOf) ·<br/>по умолчанию исключены из записи"]]
    R2 -- "отмена / закрыто" --> X2
    R2 -- "«Записать в проект (N)»" --> N["Запись ВЫБРАННОГО: требования + группы<br/>CHILD_OF · RELATES_TO из явных упоминаний<br/>идемпотентно · origin: AI_DOCS (бейдж «ИИ»)"]
...
3. **Экспорт и генерация — отдельные полноэкранные маршруты** (`/p/:id/export`,
   `/p/:id/generate`) с той же шапкой и сайдбаром, что и дерево: длинные процессы
   с выбором, параметрами и журналом не помещаются в диалог. Работает «Назад»
   браузера, состояние не теряется от клика мимо окна, из сайдбара доступны все
   разделы. Модальными остались только короткие диалоги — карточка требования,
   связь, подтверждения.
4. **Создание/редактирование** — в модальном окне с вкладками: Основное /
