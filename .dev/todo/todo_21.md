...
1) На примере архива Gitea 1.19.0.zip в Projects не работает импорт архива, выдает ошибку, что не найдены нужные openspec файлы, хотя подложив папку все прекрасно работает
2) Конпка у ФТ/НФТ для сворачивания дерева не работает пока не нажмешь на элемент "Свернуть все"
3) Неинформативное отображение ошибки Timeout при подргузке доки на AI вычленение ФТ/НФТ, при этом в виджете чата с той же моделью все прекрасно работает - ответы в чате от модели есть. Корневая проблема, что фактуры по проблеме для исправления не хватает